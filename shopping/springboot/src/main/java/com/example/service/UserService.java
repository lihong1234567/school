...
public class UserService {
    ...
    public User add(User user) {
        ...
        // 学号唯一校验
        User infoStudyNum = userDao.findByStudyNum(user.getStudyNum());
        if (ObjectUtil.isNotEmpty(info)) {
            throw new CustomException(ResultCode.USER_EXIST_ERROR);
        }
        if (ObjectUtil.isNotEmpty(infoStudyNum)) {
            throw new CustomException(ResultCode.STUDY_NUM_EXIST_ERROR);
        }
    ...
    public User numLogin(String studyNum, String mm) {
        User user = userDao.findByStudyNum(studyNum);
        if (user == null) {
